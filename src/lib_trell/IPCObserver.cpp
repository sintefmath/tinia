...
IPCObserver::IPCObserver( bool is_master )
    : m_ipc_pid( getpid() ),
      m_cleanup_pid( -1 ),
...
bool
IPCObserver::createSharedMemory( void** memory,
                                size_t* memory_size,
...
void
IPCObserver::deleteSharedMemory( void** memory, size_t* memory_size, const std::string& name )
{
    if( *memory != MAP_FAILED ) {
...
bool
IPCObserver::createSemaphore( sem_t** semaphore, const std::string& name )
{
...
void
IPCObserver::deleteSemaphore( sem_t** semaphore, const std::string& name )
{
    if( *semaphore != SEM_FAILED ) {
        sem_close( *semaphore );
    }
...
IPCObserver::~IPCObserver()
{
    std::cerr << __PRETTY_FUNCTION__ << "\n";
}

int
IPCObserver::run(int argc, char **argv)
{
...
TrellMessageType
IPCObserver::sendSmallMessage( const std::string& message_box_id, TrellMessageType query )
{
...
bool
IPCObserver::init( const std::string& xml )
{
    return true;
}
