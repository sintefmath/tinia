...
%  \draw [fill=none] (lowerleft) rectangle (upperright);
  ...
  \foreach \s in {1, ..., \pixels} {
    ...
    \ifthenelse{\s < \redpixels} {
      \draw[red, fill=red!50] (\x, 0) rectangle (\xx, \pixelheight); 
    } {
      \draw[green, fill=green!50] (\x, 0) rectangle (\xx, \pixelheight);
    }
    \pgfmathsetmacro\depthcol{abs(\redpixels-0.5-\s) * 25}
    \draw[black, fill=gray!\depthcol] (\x, \pixelheight) rectangle (\xx, 2*\pixelheight);
  }
