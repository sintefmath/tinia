...
\teaser{
  ...
  \subfigure[Server-rendered (OpenGL)]{
    \includegraphics[width=.2\linewidth]{frview1.jpg}
  }
  \subfigure[Client-rendered proxy model (Google\newline Chrome, artifacts not subdued)]{
    \includegraphics[width=.2\linewidth]{frview2.jpg}
  }
  \subfigure[Client-rendered proxy model (Google\newline Chrome, artifacts subdued)]{
    \includegraphics[width=.2\linewidth]{frview2.jpg}
  }
  \caption{\label{fig:FRView} The oil reservoir viewer FRView, developed in the
  CloudViz project~\cite{cloudviz}, showing a server-rendered image and two
  client-rendered images of automatically generated proxy models. For~(b),
  parameters are chosen to highlight some artifacts (few and large splats
  without intra-splat texturing, displaying of skewed splats) while the second
  client-rendered image (c) has parameters chosen to make an optimal proxy
  geometry (many more splats, intra-splat texturing, discarding of skewed
  splats.)}
}
...
\CCScat{\textbf{FIX THIS}Computer Graphics}{I.3.3}{Picture/Image Generation}{Line and curve generation}
\end{classification}
...
%  }
%  \caption{\label{fig:FRView} The oil reservoir viewer FRView \textbf{FIKS HER ref}, showing a
...
%  indicator. \textbf{Flytte opp til teaser, tredje bilde: uten artifacter for aa
%  vise at det kan bli bra}}
%\end{figure}
...
\marginpar{\textbf{... samle avsnittene nedenfor og komprimere...}}
