#ifndef POLICYLIB_HPP
#define POLICYLIB_HPP
...
#include "policylib/TypeToXSDType.hpp"
...
/** \mainpage
     PolicyLib is a small library used to create, update and query policies.
     ...
     Which will build a debian package which installs into /usr/lib and /usr/include/policylib .
     This package can be installed on your system with the following command
     \verbatim
        $ sudo dpkg -i PolicyLib<version>.deb \endverbatim
...
class PolicyLib {
   // The locking mechanism for PolicyLib
...
*/
   ...
   /**
     ...
     policylib::gui::Device to specify this.
     */
...
inline
std::string
PolicyLib::stringify<policylib::Viewer>( const Viewer& v ) {
   return "";
}

template<class T>
std::string
PolicyLib::stringify( const T& t ) {
   std::stringstream ss;
   ss << t;
...
template<typename T>
void
PolicyLib::addElement( std::string key, T value, const std::string annotation ) {
...
template<typename T>
void
PolicyLib::addConstrainedElement( std::string key, T value, T minConstraint, T maxConstraint, const std::string annotation ) {
...
template<class T, class TContainer>
void
PolicyLib::addElementWithRestriction( std::string key, T value, TContainer restrictions ) {
   addElementWithRestriction(key, value, restrictions.begin(), restrictions.end());
}

template<class T, class InputIterator>
void
PolicyLib::addElementWithRestriction( std::string key, T value, InputIterator start, InputIterator end) {
...
template<class T>
void
PolicyLib::updateElementHelper( std::string key, ElementData& elementData, const T& value ) {
...
template<typename T>
void
PolicyLib::updateElement( std::string key, T value ) {
...
template<typename T>
void
PolicyLib::getElementValue( std::string key, T& t ) {
...
template<typename T>
ElementData& policylib::PolicyLib::addElementInternal(std::string key,
                                                      T value)
{
...
template<class InputIterator>
void
PolicyLib::addAnnotation( std::string key, const InputIterator& begin, const InputIterator& end ) {
   using std::string;
